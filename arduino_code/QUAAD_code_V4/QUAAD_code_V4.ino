...
void setup() {
  ...
  for (int i = 0; i <= 3; i++) {
    pinMode(pin_A[i], OUTPUT); pinMode(pin_B[i], OUTPUT); 
...
void loop() {
  ...
  if (curr_clock_state == 1 & last_clock_state == 0) {
    went_up = true;
    if (clockCount < 3360) { //3360 can be divided by the numbers in divisions[]
      clockCount++;
    } else {
      clockCount = 1;
    }
  } else if (curr_clock_state == 0 & last_clock_state == 1) {
    went_down = true;
  } else { 
    went_up = false; 
    went_down = false;
